...
class PositionalEmbedding1D(nn.Module):
    ...
    def forward(self, x, patch_flag):
        """Input has shape `(batch_size, seq_len, emb_dim)`"""
        if patch_flag:
            return x + self.pos_embedding[:, 0:1, :]
        else:
...
class ViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'positional_embedding'): 
            x = self.positional_embedding(x, False)  # b,gh*gw+1,d 
            class_token = self.positional_embedding(self.class_token, True)
        if hasattr(self, 'class_token'):
            x = torch.cat((class_token.expand(b, -1, -1), x), dim=1)  # b,gh*gw+1,d
