...
class InferenceSimplifier : public MixedModeMutator {
 public:
  InferenceSimplifier(bool simplify_layer_norm=false)
      : batch_norm_op_(Op::Get("nn.batch_norm")),
        ...
        simplify_layer_norm(simplify_layer_norm) {}
  ...
  Expr Rewrite_(const CallNode* n, const Expr& new_n) {
    ...
    } else if (simplify_layer_norm && n->op == layer_norm_op_) {
 ...
 private:
  ...
  bool simplify_layer_norm {false};
  std::unordered_map<Expr, Type, ObjectPtrHash, ObjectPtrEqual> ty_map_;
};

Expr SimplifyInference(const Expr& e, bool simplify_layer_norm) { 
  return InferenceSimplifier(simplify_layer_norm).Mutate(e); 
}

namespace transform {

Pass SimplifyInference(bool simplify_layer_norm) {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        return Downcast<Function>(SimplifyInference(f, simplify_layer_norm));
      };
