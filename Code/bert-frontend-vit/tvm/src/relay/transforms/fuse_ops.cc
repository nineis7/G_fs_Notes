...
IndexedForwardGraph IndexedForwardGraph::Creator::Prepare(const Expr& body) {
...
void IndexedForwardGraph::Creator::Update(const Expr& node, IndexedForwardGraph::Node* parent, OpPatternKind pattern) {
  ...
  if (parent != nullptr) {
    // output edge: current -> parent
    auto* link = arena_->make<LinkNode<IndexedForwardGraph::Edge> >();
    link->value.node = parent;
    link->value.pattern = pattern;
    current->outputs.Push(link);
    // input edge
    link = arena_->make<LinkNode<IndexedForwardGraph::Edge> >();
    link->value.node = current;
    link->value.pattern = pattern;
    parent->inputs.Push(link);
  } else {
    current->extern_ref = true;
  }
}

void IndexedForwardGraph::Creator::AddNode(const tvm::Object* key) {
  auto it = graph_.node_map.find(key);
...
// Post order tree
void IndexedForwardGraph::Creator::VisitExpr_(const FunctionNode* op) {
...
void IndexedForwardGraph::Creator::VisitExpr_(const ConstantNode* op) {
  this->AddNode(op);
...
void IndexedForwardGraph::Creator::VisitExpr_(const CallNode* call) {
...
void IndexedForwardGraph::Creator::VisitExpr_(const TupleNode* op) {
...
void IndexedForwardGraph::Creator::VisitExpr_(const TupleGetItemNode* op) {
...
void IndexedForwardGraph::Creator::VisitExpr_(const VarNode* op) { this->AddNode(op); }

void IndexedForwardGraph::Creator::VisitExpr_(const LetNode* op) {
...
void IndexedForwardGraph::Creator::VisitExpr_(const IfNode* op) {
...
void IndexedForwardGraph::Creator::VisitExpr_(const RefCreateNode* op) {
  this->Update(op->value, nullptr, kOpaque);
  ExprVisitor::VisitExpr_(op);
  this->AddNode(op);
}

void IndexedForwardGraph::Creator::VisitExpr_(const RefReadNode* op) {
  this->Update(op->ref, nullptr, kOpaque);
  ExprVisitor::VisitExpr_(op);
  this->AddNode(op);
}

void IndexedForwardGraph::Creator::VisitExpr_(const RefWriteNode* op) {
  this->Update(op->ref, nullptr, kOpaque);
  this->Update(op->value, nullptr, kOpaque);
  ExprVisitor::VisitExpr_(op);
  this->AddNode(op);
...
void IndexedForwardGraph::Creator::VisitExpr_(const MatchNode* op) {
...
bool GroupContainOp(const std::vector<IndexedForwardGraph::Node*> gnodes, const Op& op) {
  for (auto &n: gnodes) {
    const CallNode* call = static_cast<const CallNode*>(n->ref);
    if (call->op == op) 
      return true;
  }
  return false;
