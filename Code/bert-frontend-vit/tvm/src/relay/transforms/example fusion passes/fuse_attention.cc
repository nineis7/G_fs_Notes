...
class FuseMutatorAttention : private MixedModeMutator {
 public:
    struct Group {
  ...
  Expr MakeNewFunction(Group* group, Type ret_type, Expr body) {
    ...
    func = WithAttr(std::move(func), attr::kCompiler, tvm::String("opu.attention"));
  ...
  Expr Transform(const Expr& body) {
    ...
    SearchAttentionPattern(graph);
    return this->Mutate(body);
  }

  /*
    node[17], Op(reshape) inputs=[16, ] outputs=[19, ]
    ..
    node[19], Op(nn.batch_matmul) inputs=[17, 18, ] outputs=[20, ]
    node[20], Op(reshape) inputs=[19, ] outputs=[21, ]
    node[21], Op(transpose) inputs=[20, ] outputs=[22, ]
  */
  void SearchAttentionPattern(IndexedForwardGraph &graph) {
    Group* dft = new Group();
    ...
    for (size_t i = 0; i < graph.post_dfs_order.size(); ++i) {
      ...
      if (IsOp(node->ref, Op::Get("nn.batch_matmul"))) {
        int reshape_count = 0;
        gnodes.push_back(node);

        if (IsOp(node->GetInputOuputNode(1, 0)->ref, Op::Get("reshape")))
          gnodes.push_back(node->GetInputOuputNode(1, 0));

        CollectFollowingOps(node, gnodes, reshape_count, "transpose", 1);
...
Expr FuseAttention(const Expr& expr, const IRModule& module) {
  std::cout << "\n[Fuse Attention]\n";
  return FuseMutatorAttention().Transform(expr);
}
...
Pass FuseAttention() {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        return Downcast<Function>(FuseAttention(f, m));
      };
  return CreateFunctionPass(pass_func, 0, "FuseAttention", {"InferType"});
}

TVM_REGISTER_GLOBAL("relay._transform.FuseAttention").set_body_typed(FuseAttention);
