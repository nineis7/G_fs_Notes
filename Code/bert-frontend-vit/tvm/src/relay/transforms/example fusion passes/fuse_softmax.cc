...
class FuseMutatorSoftmax : private MixedModeMutator {
 public:
    struct Group {
  ...
  Expr MakeNewFunction(Group* group, Type ret_type, Expr body) {
    ...
    func = WithAttr(std::move(func), attr::kCompiler, tvm::String("opu.key_value_matmul_softmax"));
  ...
  Expr Transform(const Expr& body) {
    ...
    SearchSoftmaxPattern(graph);
    return this->Mutate(body);
  }

  /*
    node[105], Op(nn.batch_matmul) inputs=[96, 104, ] outputs=[106, ]
    node[106], Op(reshape) inputs=[105, ] outputs=[108, ]
    node[107], const inputs=[] outputs=[108, ]
    node[108], Op(divide) inputs=[106, 107, ] outputs=[109, ]
    node[109], Op(add) inputs=[108, 39, ] outputs=[110, ]
    node[110], Op(nn.softmax) inputs=[109, ] outputs=[111, ]
  */
  void SearchSoftmaxPattern(IndexedForwardGraph &graph) {
    Group* dft = new Group();
    ...
    for (size_t i = 0; i < graph.post_dfs_order.size(); ++i) {
      ...
      if (IsOp(node->ref, Op::Get("nn.softmax"))) {
        int bmm_count = 0;
        gnodes.push_back(node);

        CollectFollowingOps(node, gnodes, bmm_count, "nn.batch_matmul", 1);
...
Expr FuseSoftmax(const Expr& expr, const IRModule& module) {
  std::cout << "\n[Fuse Softmax]\n";
  return FuseMutatorSoftmax().Transform(expr);
}
...
Pass FuseSoftmax() {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        return Downcast<Function>(FuseSoftmax(f, m));
      };
  return CreateFunctionPass(pass_func, 0, "FuseSoftmax", {"InferType"});
}

TVM_REGISTER_GLOBAL("relay._transform.FuseSoftmax").set_body_typed(FuseSoftmax);
