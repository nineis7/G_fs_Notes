...
class RelayBuildModule : public runtime::ModuleNode {
 public:
  ...
  PackedFunc GetFunction(const std::string& name, const ObjectPtr<Object>& sptr_to_self) final {
    ...
    } else if (name == "optimize_custom") {
      return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
        ICHECK_EQ(args.num_args, 2);
        *rv = this->OptimizeCustom(args[0], args[1]);
      });
    } else {
 ...
 protected:
  ...
  IRModule OptimizeCustom(IRModule relay_module, const TargetMap& targets) {
    VLOG_CONTEXT << "OptimizeCustom";
    // TODO(mbs): executor_ will be whatever was left over from last Build. Note that
    // the empty executor string will CHECK fail, so how are folks using this API?
    config_ = CompilationConfig(transform::PassContext::Current(), targets,
                                /*optional_host_target=*/Target());
    return OptimizeImplCustom(std::move(relay_module));
  }

  IRModule OptimizeImplCustom(IRModule relay_module) {
    ICHECK(relay_module.defined()) << "The IRModule must be defined for the Relay compiler.";
    ...
    // relay_module = transform::FuseEmbedding()(relay_module);
    relay_module = transform::FuseGelu()(relay_module);
    relay_module = transform::ReplaceAttentionMask()(relay_module);
    relay_module = transform::FoldConstant()(relay_module);
    relay_module = transform::Peephole("swap-divide-add")(relay_module);
    relay_module = transform::Peephole("separate-branch")(relay_module);
    relay_module = transform::FuseOpsCustom()(relay_module);
    // GenIR is dependent of ReplaceAttentionMask and FoldConstant,
    // Otherwise attention_mask_func_map_ will produce segmentation fault
    relay_module = transform::GenIR()(relay_module);
